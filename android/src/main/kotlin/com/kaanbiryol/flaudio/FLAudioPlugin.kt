    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        when (call.method) {
            Channel.prepare -> {
                ...
                AudioManager.instance.prepare(urlString, onTickHandler = { currentTime: Double, duration: Double ->
                    uiThreadHandler.post { channel.invokeMethod(Event.onTick, mapOf("time" to currentTime, "duration" to duration)) }
                }, onReadyHandler = {
                    result.success(it)
                })
            }
            ...
            Channel.playbackSpeed -> {
                val speed = arguments as? Double ?: return
                AudioManager.instance.playbackSpeed(speed)
            }
            Channel.seek -> {
                val seconds = arguments as? Double ?: return
